...
(() => {
  ...
  const AGENDA_NOTIFIED_KEY = "consulta_congreso_agenda_notified";
  const AGENDA_REMINDERS_KEY = "consulta_congreso_agenda_reminders";
  ...
  /** Interpreta día+hora del programa en hora de Argentina (UTC-3). */
  ...
  async function checkAgendaReminders() {
    ...
    for (const session of sessions) {
      ...
      // Ventana: desde 10 min antes hasta el inicio
      if (now >= start - leadMs && now < start) {
        await showAgendaNotification(session);
        markNotified(session.id);
      }
      // Si ya empezó, marcar para no reavisar al abrir tarde
  ...
  function buildIcs(sessions) {
    const eventTitle = state.data?.meta?.titulo || "Evento";
    const lines = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//Observatorio IA UCCuyo//Gestor de Eventos Científicos//ES",
      "CALSCALE:GREGORIAN",
      "METHOD:PUBLISH",
    ];
    ...
    for (const session of sessions) {
      ...
      const uid = `${session.id || icsLocal(session, "inicio")}@gestor-eventos-uccuyo`;
  ...
  function downloadIcs(sessions, filename) {
    ...
    a.href = url;
    a.download = filename || "agenda-evento.ics";
  ...
  function showAgenda() {
    ...
    if (sessions.length) {
      ...
      const remindersOn = remindersEnabled() && typeof Notification !== "undefined" && Notification.permission === "granted";
  ...
  function bindEvents() {
    ...
    els.resultsBody?.addEventListener("click", (e) => {
      ...
      if (remindBtn) {
        enableAgendaReminders().then((status) => {
          if (status === "granted") {
            ...
            window.alert(t("reminders.denied"));
          } else if (status === "unsupported") {
            window.alert(t("reminders.unsupported"));
          } else {
            showAgenda();
          }
        });
      ...
      if (icsAll) {
        downloadIcs(agendaSessions(), "mi-agenda.ics");
        return;
      }
      ...
      if (icsOne) {
        ...
        if (session) downloadIcs([session], `actividad-${sid}.ics`);
        return;
      }
