/**
 * Contador global de planes cargados en el Convertidor SACAU → CRE.
 * Incrementa al cargar un archivo con materias; muestra el total en la UI.
 */
(function (global) {
  "use strict";

  const STORAGE_TOTAL = "sacau_planes_cargados_total";
  const STORAGE_LOCAL = "sacau_planes_cargados_local";
  ...
  function paint(total, opts = {}) {
    const el = document.getElementById("sacauUsageCount");
    const wrap = document.getElementById("sacauUsage");
    if (!el) return;
    ...
    if (wrap) {
      ...
      const hint = document.getElementById("sacauUsageHint");
      if (hint) {
        hint.textContent =
          opts.source === "local"
  ...
  function fetchJsonp(url) {
    return new Promise((resolve, reject) => {
      const name = "_sacauUsageCb" + Date.now() + Math.floor(Math.random() * 1e5);
      let done = false;
  ...
  function applyRemote(data, source) {
    if (!data || !data.ok || data.planes_cargados == null) {
      throw new Error("bad_payload");
    }
    const n = Number(data.planes_cargados);
    writeCache(n);
  ...
  function debounceKey(meta) {
    ...
    return "sacau_plan_hit:" + name + ":" + size;
  }
  ...
  /**
   * Registrar un plan cargado con éxito (archivo con materias).
   * @param {{name?: string, size?: number}} meta
   */
  function recordPlanLoad(meta) {
    if (!shouldCount(meta || {})) {
      return Promise.resolve(lastShown != null ? lastShown : readCache());
    }
    bumpLocal();
    return callAction("sacau_plan")
      .then(function (data) {
        return applyRemote(data, "remote");
      })
      .catch(function () {
